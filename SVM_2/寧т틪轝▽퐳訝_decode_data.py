...
def decode_idx3_ubyte(idx3_ubyte_file):
    ...
    for i in range(num_images):
        if (i + 1) % 10000 == 0:
            print '已解析 %d' % (i + 1) + '张'
        images[i] = np.array(struct.unpack_from(fmt_image, bin_data, offset)).reshape((num_rows, num_cols))/255.0
...
def load_number_0_9_images(train = True):
    if train:
        train_images = load_train_images()
        train_labels = load_train_labels()
        flag = (train_labels ==9)|(train_labels == 8)
        train_images,train_labels =  train_images[flag],train_labels[flag]
...
def run():
    ...
    # 查看前十个数据及其标签以读取是否正确
    train_images,train_labels = laod_number_0_9_images()
    for i in range(10):
        print train_labels[i]
        print train_images[i].shape
        # print train_images[i].flatten()
        # plt.imshow(train_images[i], cmap='gray')
        # plt.show()
    print 'done'

if __name__ == '__main__':
    load_number_0_9_images()
    load_number_0_9_images(train=False)

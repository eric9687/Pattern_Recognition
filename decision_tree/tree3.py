...
# Impurity
def Impurity(dataSet):  # 这里是指香农熵，其实就是我们常说的信息熵
...
def SelectFeature(dataSet):
    ...
    baseEntropy = Impurity(dataSet)  # 计算数据集总的信息熵
    bestInfoGain = 0.0
    bestFeature = -1
...
# GenerateTree
def GenerateTree(dataSet, labels, featLabels):
    # 第一步：决定递归进程的结束和返回
    ...
    # 第二步：决定最优的分叉节点
    bestFeat = SelectFeature(dataSet)  # 这个就是算法第八行
    ...
    for value in uniqueVals:
        ...
        myTree[bestFeatLabel][value] = GenerateTree(SplitNode(dataSet, bestFeat, value), subLabels, featLabels)
    return myTree
    ...
    for key in secondDict.keys():  # 遍历所有划分属性值
        if testVec[featIndex] == key:  # 查看测试数据集划分属性对应的值
            if type(secondDict[key]).__name__ == 'dict':  #
                classLabel = Decision(secondDict[key], featLabels, testVec)  # 递归
            else:
...
if __name__ == "__main__":
    ...
    myTree = GenerateTree(myDat, labels, featLabels)
    print(myTree)
    print(" | ".join(featLabels))
    print(Decision(myTree, featLabels, ['粗', '长']))
